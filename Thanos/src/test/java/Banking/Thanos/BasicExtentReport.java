...
public class BasicExtentReport {
    ...
    public void startReport(String OS, String browser) {
        ...
        htmlReporter.config().setChartVisibilityOnOpen(true);// this code only work for extent dependency version 3.0.0
        htmlReporter.config().setDocumentTitle("Extent Report Demo"); //give the title of the document
        htmlReporter.config().setReportName("Digital Bank Test Report");//give a name for report
        htmlReporter.config().setTestViewChartLocation(ChartLocation.TOP);// set the position 
    ...
    public void getResult(ITestResult result) {
        if(result.getStatus() == ITestResult.FAILURE) {
            ...
            test.log(Status.PASS, MarkupHelper.createLabel(result.getName()+" PASSED ", ExtentColor.GREEN));//once a test case is passed, it will show in "GREEN"
        }
        else {
